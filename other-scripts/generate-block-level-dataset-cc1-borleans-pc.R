...
full_block_level_ema_dataset_prebatt_v1 <- full_block_level_ema_dataset %>% 
  ###@Tony: what records are these that we are dropping
  filter(!is.na(study_day_int)) %>% 
  group_by(participant_id) %>%
  mutate(
    ###@Tony: helpful to summarize logic here, and why is that separate from updates to these variables in the next few mutates
